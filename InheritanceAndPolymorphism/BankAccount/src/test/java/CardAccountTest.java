import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

@DisplayName("CardAccount test")
public class CardAccountTest {

    public static final double DELTA = 0.01;

    private static final String notExpectedSumMessage = "The amount on the account does not match the expected";
    private BankAccount cardAccount;

    @BeforeEach
    public void setUp() {
        cardAccount = new CardAccount();
    }

    @Test
    @DisplayName("Method 'put'")
    void put() {
        cardAccount.put(10.0);
        assertEquals(10.0, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }

    @Test
    @DisplayName("Attempt to call method 'put' with negative amount (balance should not change)")
    void putNegativeAmount() {
        cardAccount.put(-1.0);
        assertEquals(0.0, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }

    @Test
    @DisplayName("Method take, put=102.0, take=100.0")
    void put102take100() {
        cardAccount.put(102.0);
        cardAccount.take(100.0);
        assertEquals(1.0, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }

    @Test
    @DisplayName("Method take, put=2.0, take=1.0")
    void put2take1() {
        cardAccount.put(2.0);
        cardAccount.take(1.0);
        assertEquals(0.99, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }

    @Test
    @DisplayName("Method take, put=100.0, take=20.0")
    void put100take20() {
        cardAccount.put(100.0);
        cardAccount.take(20.0);
        assertEquals(79.8, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }

    @Test
    @DisplayName("Attempt to withdraw more money from the account than there is on the account")
    void takeTooMuchMoney() {
        cardAccount.put(1.0);
        cardAccount.take(3.0);
        assertEquals(1.0, cardAccount.getAmount(), DELTA, notExpectedSumMessage);
    }
}